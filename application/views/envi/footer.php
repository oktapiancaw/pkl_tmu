...
<script>
    $(document).ready(function() {
        ...
        $('#idSekolah_Kelompok').on('change', function() {
            const id = $(this).val();
            $.ajax({
                url: '<?= base_url("Jurusan_controller/getJurusan"); ?>',
                data: {
                    id: id
                },
                method: 'post',
                ...
                success: function(data) {
                    if (data.length > 0 && data !== undefined) {
                        ...
                        $('#listJurusan_kelompok').html(options)
                    } else {
                        $('#listJurusan_kelompok').html('<option>Tidak ada</option>');
                    }
